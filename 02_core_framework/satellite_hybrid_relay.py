...
HW_WILLOW = {
    "F_gate": 0.9985, "T2": 1.000, "p_gen": 0.10,
    "label": "Willow", "color": "#00A8E8"
}
HW_QUERA = {
    "F_gate": 0.9900, "T2": 0.100, "p_gen": 0.03,
    "label": "QuEra", "color": "#FF6B35"
}
HW_HELIOS = {
    "F_gate": 0.9950, "T2": 0.500, "p_gen": 0.08,
    "label": "Helios", "color": "#FFD700"
}
...
def alpha_freespace(s_km, hw_sat, hw_ground=None):
    """
    ...
    decoherence = s_km / (C_FREESPACE * t2_effective)
    wait_penalty = 2 * s_km / (C_FREESPACE * t2_effective * p)
...
def topology_fiber_only(L_km, N, hw_ground):
...
def topology_single_satellite(L_km, N_ground, hw_ground, hw_sat, 
                                sat_altitude_km=500):
    """
...
def topology_dual_satellite(L_km, N_ground, hw_ground, hw_sat,
                              sat_altitude_km=500):
    """
    ...
    sat_alpha = (2 * alpha_freespace(effective_up_down, hw_sat) +
                 alpha_freespace(inter_sat_distance, hw_sat))
...
def topology_segmented(L_km, N_total, hw_ground, hw_sat,
                        n_segments=4, sat_altitude_km=500):
    """
...
def sweep_topologies(L_km=8200, N_range=None):
    """
    ...
    for N in N_range:
        ...
        f_seg4, _ = topology_segmented(L_km, N, HW_WILLOW, HW_LEO_OPTIMISTIC, n_segments=4)
        f_seg8, _ = topology_segmented(L_km, N, HW_WILLOW, HW_LEO_OPTIMISTIC, n_segments=8)
...
def find_minimum_N_for_sage(results, topology_key):
...
def route_analysis(routes=None):
    """
...
def generate_satellite_atlas(results_8200, route_analysis_data,
                              save_path="satellite_hybrid_atlas.png"):
    """
    ...
    ax1.plot(r["N_range"], r["fiber_only"], color=RED, lw=2.5,
            label='Fiber Only (Willow)', marker='o', ms=3)
    ax1.plot(r["N_range"], r["sat_current"], color='#9C27B0', lw=1.5,
            label='+ LEO 2024 (p=0.003)', ls='--')
    ax1.plot(r["N_range"], r["sat_advanced"], color='#E040FB', lw=2.0,
            label='+ LEO 2027+ (p=0.01)', marker='s', ms=3)
    ax1.plot(r["N_range"], r["sat_optimistic"], color='#CE93D8', lw=2.5,
            label='+ LEO 2030+ (p=0.05)', marker='D', ms=3)
    ax1.plot(r["N_range"], r["dual_sat_optimistic"], color=CYAN, lw=2.0,
            label='Dual LEO 2030+', ls=':', marker='^', ms=3)

    ax1.axhline(SAGE_CONSTANT, color=GOLD, ls='--', lw=1.5, alpha=0.5)
    ax1.text(28, SAGE_CONSTANT + 0.02, f'Sage ({SAGE_CONSTANT})',
            color=GOLD, fontsize=8, ha='right', fontfamily='monospace')
    ...
    ax1.legend(fontsize=7, facecolor=BG, edgecolor=GRID,
    ...
    bars1 = ax2.bar(x - width/2, fiber_f, width, color=RED, alpha=0.8,
                   label='Fiber Only')
    bars2 = ax2.bar(x + width/2, sat_f, width, color='#CE93D8', alpha=0.8,
                   label='Sat-Hybrid (2030+)')

    ax2.axhline(SAGE_CONSTANT, color=GOLD, ls='--', lw=1.5, alpha=0.5)
    ax2.set_xticks(x)
    ax2.set_xticklabels([f'{r}\n({d:,} km)' for r, d in zip(routes, distances)],
                        fontsize=7, color=WHITE)
    ax2.set_ylabel('Best Achievable Fidelity (N=30)', color=WHITE, fontsize=9)
    ax2.legend(fontsize=8, facecolor=BG, edgecolor=GRID,
    ...
    for i, route in enumerate(routes):
        ...
        if rd["sat_feasible"]:
            ax2.text(i + width/2, rd["best_sat_f"] + 0.02, 'OK',
                    ha='center', color=CYAN, fontsize=8, fontweight='bold')
        else:
            ax2.text(i + width/2, rd["best_sat_f"] + 0.02, 'GAP',
    ...
    for hw, marker in [(HW_LEO_CURRENT, 'o'), (HW_LEO_ADVANCED, 's'),
                       (HW_LEO_OPTIMISTIC, 'D')]:
        f_point, _ = topology_single_satellite(L_fixed, N_fixed, HW_WILLOW, hw)
        ax3.plot(hw["p_gen"], f_point, marker=marker, ms=12,
                color=hw["color"], markeredgecolor=WHITE,
                markeredgewidth=1.5, zorder=10)
        ax3.text(hw["p_gen"] * 1.3, f_point, hw["label"],
                color=hw["color"], fontsize=8, fontfamily='monospace',
                va='center')
    ...
    for i, (p, f) in enumerate(zip(p_gen_values, fidelities_by_p)):
        if f >= SAGE_CONSTANT:
            ...
            ax3.text(p, 0.1, f'p*={p:.3f}', color=GOLD, fontsize=8,
                    fontfamily='monospace', rotation=90, va='bottom')
            break

    # ------------------------------------------------------------------
    # PANEL 4: Min N Required Per Route (The Identity Bridge)
    # ------------------------------------------------------------------
    ...
    if unreachable_d:
        ax4.axvspan(min(unreachable_d), max(unreachable_d),
                   alpha=0.1, color=RED)
        ax4.text(np.mean(unreachable_d), 50, 'UNREACHABLE\n(with LEO 2030+)',
    ...
    for name, rd in route_analysis_data.items():
        if rd["min_N_sat_opt"] is not None:
            ax4.plot(rd["distance_km"], rd["min_N_sat_opt"], 'D',
                    color=GOLD, ms=10, markeredgecolor=WHITE,
                    markeredgewidth=1.5, zorder=10)
            ax4.text(rd["distance_km"] + 200, rd["min_N_sat_opt"],
                    f'{name}\n(N={rd["min_N_sat_opt"]})',
...
if __name__ == '__main__':
    ...
    for key, label in [("fiber_only", "Fiber Only"),
    ...
    for name, rd in routes.items():
        best = max(rd["best_fiber_f"], rd["best_sat_f"],
                   rd["best_seg4_f"], rd["best_seg8_f"])
        feasible = 'YES' if best >= SAGE_CONSTANT else 'NO'
        print(f'  {name:<20s} {rd["distance_km"]:>6,d}km'
