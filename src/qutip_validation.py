...
try:
    ...
    print(
        "[WARNING] numpy or matplotlib not installed. Run: pip install numpy matplotlib"
    )
...
HW_QUERA = {
    "name": "QuEra-class",
    "F_gate": 0.9920,  # Corrected for consistency with paper draft
    "T2": 0.100,
    "p_gen": 0.03,
}
...
def depolarizing_channel(rho, p):
    ...
    """
    # dimension per qubit (not used, but kept for clarity if needed later)
    # _d = rho.dims[0][0]
    ...
    # Apply to qubit 1
    rho_out = (1 - p) * rho + (p / 3) * (
        sx1 * rho * sx1.dag() + sy1 * rho * sy1.dag() + sz1 * rho * sz1.dag()
    )
...
def validate_sage_vs_qutip(N_range=None, L_km=8200, hw=None):
    ...
    for N in N_range:
        ...
        f_sage = max(0.0, min(1.0, float(f_sage)))
...
def generate_validation_atlas(results_willow, results_quera=None):
    ...
    fig, axes_raw = plt.subplots(1, n_panels, figsize=(6 * n_panels, 5), facecolor=BG)
    axes = cast(
        Any, axes_raw
    )  # Cast for type checker, as axes can be a single Axes object or an array

    fig.suptitle(
        "QUTIP VALIDATION — SAGE Analytical vs Density Matrix Evolution",
        color=GOLD,
        fontsize=14,
        fontweight="bold",
        y=1.02,
    ...
    if not isinstance(axes, (list, np.ndarray)):
        ...
        # Safely flatten the axes object
        if hasattr(axes, "ravel"):
            ax_list = list(cast(Any, axes).ravel())
    ...
    ax.plot(
        results_willow["N_range"],
        results_willow["sage_fidelities"],
        color=CYAN,
        lw=2.5,
        marker="o",
        ms=4,
    ...
    if results_willow["qutip_fidelities"][0] is not None:
        ax.plot(
            results_willow["N_range"],
            results_willow["qutip_fidelities"],
            color=ORANGE,
            lw=2.5,
            marker="s",
            ms=4,
            label="QuTiP Density Matrix",
        )
    ax.axhline(SAGE_CONSTANT, color=GOLD, ls="--", lw=1.2, alpha=0.5)
    ax.set_title(
        f"Fidelity: {results_willow['hw_name']} ({results_willow['L_km']:,} km)",
        color=WHITE,
        fontsize=10,
        pad=6,
    )
    ax.set_xlabel("Repeater Nodes (N)")
    ...
    valid_errors = [
        (n, e)
        for n, e in zip(results_willow["N_range"], results_willow["relative_errors"])
        if e is not None
    ]
    if valid_errors:
    ...
    ax2.set_xlabel("Repeater Nodes (N)")
    ax2.set_ylabel("|SAGE - QuTiP| / QuTiP (%)")
    ...
    if results_quera and n_panels == 3:
        ...
        ax3.plot(
            results_quera["N_range"],
            results_quera["sage_fidelities"],
            color=CYAN,
            lw=2.5,
            marker="o",
            ms=4,
        ...
        if results_quera["qutip_fidelities"][0] is not None:
            ax3.plot(
                results_quera["N_range"],
                results_quera["qutip_fidelities"],
                color=ORANGE,
                lw=2.5,
                marker="s",
                ms=4,
        ...
        ax3.set_title(
            f"Fidelity: {results_quera['hw_name']} ({results_quera['L_km']:,} km)",
            color=WHITE,
            fontsize=10,
            pad=6,
        )
        ax3.set_xlabel("Repeater Nodes (N)")
        ax3.set_ylabel("End-to-End Fidelity")
...
if __name__ == "__main__":
    ...
    print("\n  QuTiP version: " + str(qutip.__version__))
    print("  Route: Beijing-London, L = 8,200 km")
