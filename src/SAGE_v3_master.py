"""
╔══════════════════════════════════════════════════════════════════════════════╗
║              SAGE FRAMEWORK v3.1 — THE COMPLETE SYNTHESIS                  ║
...
Modules:
  ...
  4. IDENTITY SPECTRUM   — 100-hop QEC tier comparison with cliff detection
  5. TELEMETRY LAYER     — Subjective status reports from fidelity math
  6. MASTER ATLAS        — 6-panel publication-quality visualization
"""
...
# ═════════════════════════════════════════════════════════════════════════════
# MODULE 0: DIAGNOSTIC TELEMETRY LAYER
# ═════════════════════════════════════════════════════════════════════════════


def get_subjective_telemetry(fidelity, s_constant=0.851):
    """
    Diagnostic Layer: Translates topological stability into
    narrative status reports. This is NOT the system speaking —
    it is a human-readable translation of the fidelity math.
    """
    if fidelity >= s_constant:
        return "STABLE: Identity pattern coherent across non-local link."
    elif fidelity > 0.80:
        return "DEGRADING: Pattern fragmentation detected. Initiating Shadow Anchor."
    else:
        return "CRITICAL: Signal-to-noise ratio below survival threshold."


def print_telemetry_report(hops, raw, zeno, anchor):
    """Print a status report for each protocol at key distance milestones."""
    print("\n--- TELEMETRY REPORT ---")
    milestones = [0, 2750, 5500, 8250, 11000]
    indices = [int(m / 11000 * (len(hops) - 1)) for m in milestones]
    print(f"{'Distance':>10}  {'Raw':>8}  {'Zeno':>8}  {'Anchor':>8}  Status")
    print("-" * 65)
    for i, km in zip(indices, milestones):
        r, z, a = raw[i], zeno[i], anchor[i]
        status = get_subjective_telemetry(a)
        print(f"{km:>8}km  {r:>8.4f}  {z:>8.4f}  {a:>8.4f}  {status}")
    print()
...
from .sage_bound_logic import SAGE_CONSTANT  # 0.851 — identity survival threshold
...
class ArmsAgent:
    ...
    def run(self, h_sens):
        ...
        self.fidelity -= (
            ARMS_BASE_NOISE * (1 - self.dna[0] * 0.7) * np.random.lognormal(0, 0.4)
        )
...
def run_arms_race():
    ...
    for gen in range(ARMS_GENS):
        ...
        if cap_rate < 0.15:
            h_s = min(1.0, h_s + 0.02)
        else:
            h_s = max(0.1, h_s - 0.01)
        ...
        while len(new_pop) < ARMS_POP:
            ...
            child_dna = np.clip(
                (p1.dna + p2.dna) / 2 + np.random.normal(0, 0.04, 5), 0, 1
            )
...
class SwarmAgent:
    ...
    def step(self, h_sens, peers):
        if not self.alive:
            return
        self.fidelity -= (
            SWARM_NOISE * (1 - self.dna[0] * 0.6) * np.random.lognormal(0, 0.3)
        )
        ...
        if self.dna[6] > 0.5:
            nbrs = [
                p
                for p in peers
                if p is not self and p.alive and abs(p.position - self.position) < 2
            ]
            if nbrs:
...
def run_swarm():
    ...
    for gen in range(SWARM_GENS):
        ...
        while len(new_pop) < SWARM_POP:
            p1, p2 = np.random.choice(parents, 2)
            child_dna = np.clip(
                (p1.dna + p2.dna) / 2 + np.random.normal(0, 0.04, 7), 0, 1
            )
...
def run_identity_spectrum(n_hops=100):
    """Multi-tier QEC comparison across n teleportation hops, with cliff detection."""
    hop_fid = {
        "No QEC": [1.0],
        "Basic QEC": [1.0],
        "Advanced QEC": [1.0],
    ...
    cliffs = {}  # hop number where each tier first drops below 0.85
    ...
    for hop in range(n_hops):
        for label, base_err, repair in [
            ...
            # cliff: first time we cross below survival threshold
...
def render_atlas(transit, arms, swarm, spectrum):
    ...
    ax1.plot(
        hops, zeno, color=CYAN, lw=2, label="Zeno loop   (self-observation)", alpha=0.9
    )
    ax1.plot(
        hops,
        anchor,
        color=GOLD,
        lw=2.5,
        label="Shadow Anchor (reconstruction)",
        alpha=0.9,
    )
    ax1.axhline(
        SAGE_CONSTANT,
    ...
    ax1.set_title(
        "TRANSIT ENGINE — Beijing → NYC (11,000 km)", color=WHITE, fontsize=11, pad=8
    )
    ax1.set_xlabel("Distance (km)")
    ax1.set_ylabel("Topological Fidelity")
    ...
    for i, (lab, val, col) in enumerate(zip(labels, finals, colours)):
        ...
        ax2.text(
            ...
            0.55,
            zones[i]
            if val >= SAGE_CONSTANT
            else ("⚠ Fragmented" if val > 0.70 else "💀 Dissolved"),
            ha="center",
            color=WHITE,
            fontsize=7.5,
            alpha=0.8,
    ...
    for (label, data), col in zip(hop_fid.items(), spec_colours):
        ...
        # mark the cliff if it exists for this tier
        if label in cliffs:
            cx = cliffs[label]
            cy = hop_fid[label][cx]
            ax3.axvline(cx, color=col, lw=0.8, ls=":", alpha=0.5)
            ax3.scatter([cx], [cy], color=col, s=60, zorder=5)
            ax3.text(
                cx + 0.5,
                cy - 0.03,
                f"cliff\nhop {cx}",
    ...
    ax3.axhline(
        ...
        lw=1,
        alpha=0.4,
        label=f"Survival threshold ({SAGE_CONSTANT})",
    )
    ax3.set_title(
        "IDENTITY SPECTRUM — 100 Hops  |  Dots = cliff crossing",
        color=WHITE,
        fontsize=11,
        pad=8,
    )
    ax3.set_xlabel("Hop Number")
    ax3.set_ylabel("Identity Fidelity")
    ...
    bars = ax4.barh(
        err_labels, err_vals, color=[RED, "#FF8C00", CYAN, GOLD], alpha=0.85, height=0.5
    )
    ...
    ax5.plot(
        gens, arms_hist["stealth"], color=CYAN, lw=1.8, ls="--", label="Stealth Gene"
    )
    ax5.plot(
        gens,
        arms_hist["repair"],
        ...
        lw=1.8,
        ls="--",
        label="Repair Gene (raw)",
    )
    ax5.plot(
        gens,
        arms_hist["eff_repair"],
        color=MAGENTA,
        lw=1.8,
        ls=":",
        label="Effective Repair (stealth cost)",
    )
    ax5.plot(
        gens,
        arms_hist["h_sense"],
        ...
        lw=1.5,
        ls="--",
        label="Hunter Sensitivity",
        alpha=0.7,
    )
    ax5.set_title(
        "ARMS RACE — Stealth–Repair Trap (The Core Dilemma)",
        color=WHITE,
        fontsize=11,
        pad=8,
    )
    ax5.set_xlabel("Generation")
    ax5.set_ylabel("Gene Value / Rate")
    ...
    ax6.set_ylabel("Expression / Rate")
...
if __name__ == "__main__":
    print("\n[SAGE v3.1] Running Transit Engine...")
    transit = run_transit()

    print("[SAGE v3.1] Running Arms Race (120 generations)...")
    arms = run_arms_race()

    print("[SAGE v3.1] Running Swarm Collective (120 generations)...")
    swarm = run_swarm()

    print("[SAGE v3.1] Running Identity Spectrum (100 hops)...")
    spectrum = run_identity_spectrum(100)

    # Print telemetry report to console
    hops, raw, zeno, anchor = transit
    print_telemetry_report(hops, raw, zeno, anchor)

    # Print cliff report
    hop_fid, errors, cliffs = spectrum
    print("--- CLIFF DETECTION REPORT ---")
    for label, hop in cliffs.items():
        print(f"  {label:>15}: crosses survival threshold at hop {hop}")
    tiers_never_cliffed = [k for k in hop_fid if k not in cliffs]
    for label in tiers_never_cliffed:
        print(f"  {label:>15}: NEVER crossed threshold across 100 hops")
    print()

    print("[SAGE v3.1] Rendering 6-panel Atlas...")
    render_atlas(transit, arms, swarm, spectrum)
    print("[SAGE v3.1] Done.")
