...
STAGE_CONFIGS = {
    1: {
        "name": "Decoherence Resistance",
        "base_noise": 0.025,
        "hunters": False,
        "whisper_req": False,
        "label": "Stage 1: Physics Only",
    },
    2: {
        ...
        "hunters": True,
        "whisper_req": False,
        "label": "Stage 2: Arms Race",
    },
    3: {
        "name": "The Whisper Engine",
        "base_noise": 0.025,
        "hunters": True,
        "whisper_req": True,
        "label": "Stage 3: Identity",
    },
    4: {
    ...
    def step(self, h_sens, peers, config):
        ...
        if self.dna[6] > 0.4:
            neighbors = [
                p
                for p in peers
                if p is not self and p.alive and abs(p.position - self.position) < 3.0
            ]
            if neighbors:
...
def run_all_stages(pop_size=250, generations=120, seed=42):
    ...
    for stage in range(1, 5):
        ...
        print(
            f"    -> Survival={final_surv:.0%}  Sync={final_sync:.2f}  Whisper={final_whisper:.2f}"
        )
...
def generate_singularity_atlas(results, save_path="singularity_protocol_atlas.png"):
    ...
    subtitle = (
        "Decoherence Resistance -> Ghost Protocol -> Whisper Engine -> Quantum Winter"
    )
    fig.text(
        0.5,
        0.955,
    ...
    for stage in [1, 2, 3, 4]:
        ...
        if stage < 4:
            ax1.axvline(
                x=offset + len(hist["sync"]), color=GRID, linestyle="--", alpha=0.4
            )
    ...
    if transition_gen is not None:
        ax1.axvline(
            x=360 + transition_gen, color=RED, linestyle=":", alpha=0.8, linewidth=1.5
        )
        ax1.text(
            360 + transition_gen + 3,
            0.85,
            "Phase\nTransition",
            color=RED,
            fontsize=7,
    ...
    ax1.set_xticklabels(
        ["S1: Physics", "S2: Ghost", "S3: Whisper", "S4: Singularity"],
        fontsize=7,
        color=WHITE,
    )
    ax1.legend(
    ...
    ax2.text(
        115, 0.87, "86%", color=GOLD, fontsize=8, ha="right", fontfamily="monospace"
    )
    ...
    ax4_twin = ax4.twinx()
    ax4_twin.plot(
        x, hist4["sync"], color=CYAN, linewidth=2.5, linestyle="-", label="Sync Gene"
    )
    ax4_twin.plot(
        x,
        hist4["whisper"],
...
if __name__ == "__main__":
    ...
    for stage in range(1, 5):
        ...
        print(
            f"    Stealth:  {s['stealth'][-1]:.2f}  |  Repair:  {s['repair'][-1]:.2f}"
        )
        if cfg["hunters"]:
