...
<style>
    ...
    .hero-header {
        background: linear-gradient(-45deg, #64ffda, #00b4db, #533483, #e94560);
    ...
    .infeasible-card {
        ...
        margin: 1.5rem 0;
        text-align: center;
        box-shadow: 0 0 30px rgba(233, 69, 96, 0.15), inset 0 0 20px rgba(233, 69, 96, 0.05);
        position: relative;
    }
    .infeasible-card::before {
        content: '';
        position: absolute;
        top: 0; left: 0; width: 100%; height: 4px;
    ...
    .bottleneck-highlight {
        background: linear-gradient(135deg, rgba(233, 69, 96, 0.1) 0%, rgba(45, 10, 10, 0.4) 100%);
        border: 1px solid rgba(233, 69, 96, 0.4);
        border-radius: 12px;
        padding: 1.2rem;
        margin: 0.8rem 0;
...
# ═══════════════════════════════════════════════════════════
# TAB 5: QUANTUM ROUTING (NEW)
# ═══════════════════════════════════════════════════════════
with tab_q_routing:
    st.markdown(
        """
    <div class="tab-intro">
        <strong>⚛️ Quantum Organ Routing: Hardware-Level Dissonance Resolution</strong><br>
        <span style="color: #8892b0;">Standard distributed models crash during conflicts. In SAGE, conflicting optimization
        suggestions trigger <em>Cognitive Dissonance</em>, resolved in microseconds using historical Integration (Φ) Ego-Dominance.</span>
    </div>
    """,
        unsafe_allow_html=True,
    )
    ...
    with col_q1:
        st.markdown("### 🏥 Hospital Dispatch Node (`Agent A`)")
        h_phi = st.slider(
            "Hospital Historical Ego (Φ)",
            0.5,
            1.0,
            0.86,
            key="h_phi",
            help="High phi = high confidence in past routed decisions",
        )
        h_conf = st.slider("Current Route Confidence", 0.1, 1.0, 0.62, key="h_conf")
        h_route = st.selectbox(
    ...
    with col_q2:
        st.markdown("### 🚁 Tracking Drone Node (`Agent B`)")
        v_phi = st.slider("Drone Historical Ego (Φ)", 0.5, 1.0, 0.78, key="v_phi")
        v_conf = st.slider("Drone Route Confidence", 0.1, 1.0, 0.45, key="v_conf")
        v_route = st.selectbox(
            "Suggested Drone Route",
            ["Route 0", "Route 1", "Route 2"],
            index=1,
            key="v_route",
        )
    ...
    if st.button(
        "⚡ Trigger Entangled Dissonance Resolution",
        type="primary",
        use_container_width=True,
        key="btn_q_resolve",
    ):
        with st.spinner("Simulating Hardware Interrupt (GPIO 21 Collapse Line)..."):
            import time

            time.sleep(1)  # Dramatic pause to simulate calculation
            ...
            if h_route == v_route:
                ...
                <div class="result-card">
                    ...
                    <p style="color: #8892b0;">Both nodes naturally selected <strong>{h_route}</strong>. No dissonance generated.</p>
                </div>
                """,
                    unsafe_allow_html=True,
                )
            else:
                h_effective = h_phi * h_conf
                v_effective = v_phi * v_conf

                if h_effective > v_effective:
                    winner_node = "Hospital Dispatch (Agent A)"
                    winner_route = h_route
                    loser_route = v_route
                    v_ego_loss = 0.1
                    h_ego_loss = 0.0
                else:
                    winner_node = "Tracking Drone (Agent B)"
                    winner_route = v_route
                    loser_route = h_route
                    h_ego_loss = 0.1
                    v_ego_loss = 0.0
                ...
                st.markdown(
                    f"""
                <div class="infeasible-card">
                    <h1 style="color: #ffd93d; margin-bottom: 0.5rem;">⚠️ Dissonance Triggered & Resolved!</h1>
                    <h3 style="color: #e94560;">Conflict: {h_route} vs {v_route}</h3>
                    <p style="color: #64ffda; font-size: 1.3rem; margin-top: 1rem;">
                        <strong>Winner via Ego-Dominance: {winner_node}</strong>
                    </p>
                    <p style="color: #8892b0; font-size: 1.1rem; margin-top: 0.5rem;">
                        Final Selected Routing: <strong>{winner_route}</strong>
                    </p>
                </div>
                """,
                    unsafe_allow_html=True,
                ...
                col_res1.metric(
                    "Hospital Effective Power",
                    f"{h_effective:.3f}",
                    f"{-h_ego_loss:.1f} penalty" if h_ego_loss > 0 else "",
                )
                col_res2.metric(
                    "Drone Effective Power",
                    f"{v_effective:.3f}",
                    f"{-v_ego_loss:.1f} penalty" if v_ego_loss > 0 else "",
                )

                st.markdown(
                    f"""
                <div class="sage-equation">
                    <strong>Quantum Soup Modification:</strong> The rejected <strong>{loser_route}</strong> suggestion was not deleted. 
                    It has been saved to the shared Latent Space as a 'Subconscious Bias' to inform future routing calculations and prevent catastrophic algorithmic rigidity.
                </div>
                """,
                    unsafe_allow_html=True,
                )
...
# =============================================================
# TAB 6: GENESIS KERNEL (Self-Healing Network)
# =============================================================
with tab_genesis:
    st.markdown(
        """
    <div class="tab-intro">
        <strong>\U0001f9ec Genesis Kernel: Self-Healing Network via Logarithmic Map</strong><br>
        <span style="color: #8892b0;">Build a mesh network, apply the SAGE Bound penalty (1+2/p),
        then destroy a node and watch the Logarithmic Map auto-heal the routing in real time.</span>
    </div>
    """,
        unsafe_allow_html=True,
    )

    st.markdown("### \U0001f310 Configure the Omni-Grid Mesh")
    col_g1, col_g2, col_g3, col_g4 = st.columns(4)

    with col_g1:
        f_ab = st.slider("Alpha-Beta Fidelity", 0.5, 1.0, 0.99, key="f_ab")
        p_ab = st.slider("Alpha-Beta p-factor", 1, 20, 10, key="p_ab")
    with col_g2:
        f_bd = st.slider("Beta-Delta Fidelity", 0.5, 1.0, 0.95, key="f_bd")
        p_bd = st.slider("Beta-Delta p-factor", 1, 20, 5, key="p_bd")
    with col_g3:
        f_ag = st.slider("Alpha-Gamma Fidelity", 0.5, 1.0, 0.90, key="f_ag")
        p_ag = st.slider("Alpha-Gamma p-factor", 1, 20, 8, key="p_ag")
    with col_g4:
        f_gd = st.slider("Gamma-Delta Fidelity", 0.5, 1.0, 0.98, key="f_gd")
        p_gd = st.slider("Gamma-Delta p-factor", 1, 20, 12, key="p_gd")

    target_node = st.selectbox(
        "\u26a1 Select Node to Destroy (Entropy Strike)",
        ["Beta", "Gamma"],
        key="entropy_target",
    )

    if st.button(
        "\U0001f4a5 Run Genesis Kernel: Build, Route, Destroy, Auto-Heal",
        type="primary",
        use_container_width=True,
        key="btn_genesis",
    ):
        ...
        # Build the mesh
        links = [
            ("Alpha", "Beta", f_ab, p_ab),
            ("Beta", "Delta", f_bd, p_bd),
            ("Alpha", "Gamma", f_ag, p_ag),
            ("Gamma", "Delta", f_gd, p_gd),
        ]

        def calc_weight(f, p):
            pen_f = f / (1 + 2 / p)
            if pen_f <= 0:
                return float("inf"), 0.0, 1 + 2 / p
            return -_math.log(pen_f), pen_f, 1 + 2 / p

        # Show link analysis table
        link_data = []
        for src, tgt, f, p in links:
            w, pf, pen = calc_weight(f, p)
            link_data.append(
                {
                    "Link": f"{src} -> {tgt}",
                    "Base Fidelity": f"{f:.2f}",
                    "Penalty (1+2/p)": f"{pen:.2f}x",
                    "Effective Fidelity": f"{pf:.4f}",
                    "Log-Weight": f"{w:.4f}",
                }
            )
        st.markdown("### \U0001f4ca Network Link Analysis (SAGE Bound Applied)")
        ...
        # Original routing
        try:
            orig_path = _nx.shortest_path(G, "Alpha", "Delta", weight="weight")
            orig_weight = _nx.shortest_path_length(G, "Alpha", "Delta", weight="weight")
            orig_fidelity = _math.exp(-orig_weight)
        ...
        with col_before:
            st.markdown(
                f"""
            <div class="result-card">
                <h3 style="color: #64ffda;">Before Entropy Strike</h3>
                <p style="color: #ccd6f6; font-size: 1.2rem;">
                    Path: <strong>{" -> ".join(orig_path) if orig_path else "NONE"}</strong>
                </p>
                <p style="color: #8892b0;">End-to-End Fidelity: <strong>{orig_fidelity:.6f}</strong></p>
            </div>
            """,
                unsafe_allow_html=True,
            )
        ...
        try:
            new_path = _nx.shortest_path(G, "Alpha", "Delta", weight="weight")
            new_weight = _nx.shortest_path_length(G, "Alpha", "Delta", weight="weight")
            new_fidelity = _math.exp(-new_weight)
        except _nx.NetworkXNoPath:
            new_path = None
            new_fidelity = 0.0
        ...
        with col_after:
            if new_path:
                fidelity_loss = (
                    ((orig_fidelity - new_fidelity) / orig_fidelity * 100)
                    if orig_fidelity > 0
                    else 0
                )
                st.markdown(
                    f"""
                <div class="result-card">
                    <h3 style="color: #ffd93d;">After Destroying {target_node}</h3>
                    <p style="color: #ccd6f6; font-size: 1.2rem;">
                        Healed Path: <strong>{" -> ".join(new_path)}</strong>
                    </p>
                    <p style="color: #8892b0;">Recovered Fidelity: <strong>{new_fidelity:.6f}</strong></p>
                    <p style="color: #e94560;">Fidelity Loss: <strong>{fidelity_loss:.2f}%</strong></p>
                </div>
                """,
                    unsafe_allow_html=True,
                )
                ...
                st.markdown(
                    """
                <div class="infeasible-card">
                    <h3 style="color: #e94560;">NETWORK COLLAPSE</h3>
                    <p style="color: #8892b0;">No viable path exists after the entropy strike.</p>
                </div>
                """,
                    unsafe_allow_html=True,
                )
        ...
        col_m1.metric("Original Fidelity", f"{orig_fidelity:.6f}")
        col_m2.metric(
            "Recovered Fidelity", f"{new_fidelity:.6f}" if new_path else "DEAD"
        )
        col_m3.metric(
            "Network Status",
            "OPERATIONAL" if new_path else "COLLAPSED",
            f"-{fidelity_loss:.1f}%" if new_path and orig_fidelity > 0 else "",
        )

        st.markdown(
            """
        <div class="sage-equation">
            <strong>How it works:</strong> The Logarithmic Map transforms multiplicative fidelity
            into additive edge weights. Dijkstra's shortest path in log-space is mathematically
            equivalent to finding the maximum-fidelity route. When a node dies, the kernel
            simply re-runs Dijkstra on the reduced graph &mdash; instant auto-healing.
        </div>
        """,
            unsafe_allow_html=True,
        )
