...
def render_atlas(transit, arms, swarm, spectrum):
    ...
    for (label, data), col in zip(hop_fid.items(), spec_colours):
        ...
        if label in cliffs:
            ...
            ax3.text(cx+0.5, cy-0.03, f'cliff\nhop {cx}',
                     color=col, fontsize=7, alpha=0.85)
    ax3.axhline(SAGE_CONSTANT, color=WHITE, ls='--', lw=1, alpha=0.4,
                label=f'Survival threshold ({SAGE_CONSTANT})')
    ax3.set_title('IDENTITY SPECTRUM — 100 Hops  |  Dots = cliff crossing', color=WHITE, fontsize=11, pad=8)
