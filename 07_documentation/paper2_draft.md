...
- **Claude 3.5 Sonnet / Claude 3 Opus** (Anthropic): Primary code generation, mathematical reasoning
- **Gemini 2.5 Pro** (Google): Cross-domain synthesis, validation
- **Python ecosystem**: NumPy, Matplotlib, SciPy, Cirq, QuTiP
- **Total development time**: ~12 hours across multiple sessions
- **Lines of code generated**: ~5,000 (framework) + ~2,000 (validation + upgrades)
- **Modules produced**: 8 Python modules, 7 atlas visualizations
- **Theorems stated and proved**: 4 (all independently validated)
- **Honest negatives documented**: 2 (quorum sensing, consciousness framing)
...
The most novel cross-disciplinary connection in this work is the **formal mapping between MI feature steering and quantum hardware optimization**. This is not an analogy — it is a theorem. The residual stream decomposition h_L = h₀ + Σ Δh_i (Elhage et al. [6]) has an exact quantum analogue:

| Anthropic (Neural) | SAGE (Quantum) |
|--------------------|-----------|
| Feature = direction in latent space | Hardware = point in parameter space |
| Residual stream: h_L = h₀ + Σ Δh_i | Log-fidelity: log(F) = Σ α_i |
| Steer: clamp Golden Gate feature at +*k* | Steer: clamp node 3 F_gate at *x* |
| Response: ≈linear in *k* | Response: **exactly** linear in log(*x*) |
| Mechanism: residual stream additivity | Mechanism: monoid homomorphism |
| R²: approximate (empirical) | R²: 1.0000000000 (exact theorem) |
...
2. **Quantum → MI**: The fact that quantum steering is *exactly* linear (R² = 1.0) while neural steering is *approximately* linear suggests that the nonlinearity in neural networks' feature steering comes from the irreducible nonlinearities (ReLU, softmax) not captured by the residual stream model. The quantum case provides a "noise-free" reference point for understanding where the neural approximation breaks down.
...
**Test**: Fit the Hill function V = V_max · x^n / (K^n + x^n) to the smoothed Sync gene trajectory and compare Hill coefficient *n* to four biological species.

**Result**: n = 8.0 (biological range: 1.5–3.0), R² = −0.305, 0/4 matches.
...
This negative took 30 minutes to establish definitively. Without the AI, the researcher might have spent weeks building an increasingly elaborate analogy before discovering it doesn't hold.
...
- Clamping one node's F_gate and sweeping from 0.970 → 0.999
- Observing the LP optimum's response
- **Result**: slope = 2.000000 (exact prediction from theory), R² = 1.0

This is the quantum analogue of Anthropic's Golden Gate Bridge experiment [7]. A precise terminological clarification is needed: the exact linearity holds **in the log-transformed LP space**, not in the raw fidelity space. Clamping raw F_gate and sweeping it produces a multiplicative change in F_total. Under the monoid homomorphism φ = log, this multiplicative change maps to an additive (linear) change in log(F_total). The exact relationship is:

$$\frac{\partial \log(F_{\text{total}})}{\partial \log(F_{\text{gate},j})} = 2 \quad \text{(exactly, for all } j\text{)}$$

The slope of 2 reflects the two Bell measurements per hop. This is exact because φ is an exact homomorphism, unlike neural residual stream additivity where the analogous derivative is approximately but not exactly constant.

The comparison to Anthropic's work is instructive: in the neural case, clamping a feature direction produces an *approximately* linear perturbation because the residual stream model h_L = h₀ + Σ Δh_i is an approximation — nonlinear operations (LayerNorm, softmax) introduce deviations. In the quantum case, the log-additivity is *exact* because no nonlinear operations intervene between hops. The quantum network thus serves as a "noise-free" reference point for understanding where the neural approximation breaks down.

Crucially, this is a **testable prediction**: if the monoid homomorphism is correct, then the sensitivity of quantum network fidelity to any single node's hardware should be exactly 2.0 (the number of gates per hop). Our simulation confirms this to machine precision.
...
The 12×–40× speedups observed in this case study (12 hours of AI-assisted work vs estimated weeks-to-months of specialist time for equivalent scope) raise questions:

- **Accessibility**: Non-specialists can now produce validated results in specialized fields. This democratizes research but increases the need for rigorous validation frameworks.
- **Attribution**: How should AI contributions be credited? We chose transparency over co-authorship.
- **Reproducibility**: AI-assisted research produces more complete reproducibility packages because the code is generated alongside the analysis, not retrofitted afterward.
...
The most important meta-finding may be that AI assistance is most valuable not when it confirms the researcher's intuitions but when it honestly identifies where those intuitions fail. The quorum sensing negative, the consciousness framing separation, and the technology gap characterization were all cases where the AI's critical evaluation was more valuable than its generative capability.
...
[2] Romera-Paredes, B. et al. (2024). "Mathematical discoveries from program search with large language models." *Nature*, 625, 468–475.

[3] Johansson, J.R. et al. (2013). "QuTiP 2: A Python framework for the dynamics of open quantum systems." *Computer Physics Communications*, 184, 1234–1240.

[4] Chen, Y.-A. et al. (2021). "An integrated space-to-ground quantum communication network over 4,600 kilometres." *Nature*, 589, 214–219.

[5] Beckmann, P. & Queloz, M. (2026). "Mechanistic Indicators of Understanding in Large Language Models." *arXiv:2507.08017v4*.

[6] Kimble, H.J. (2008). "The quantum internet." *Nature*, 453, 1023–1030.

[7] Templeton, A. et al. (2024). "Scaling monosemanticity: Extracting interpretable features from Claude 3 Sonnet." *Anthropic Research*.

[8] Power, A. et al. (2022). "Grokking: Generalization beyond overfitting on small algorithmic datasets." *arXiv:2201.02177*.

[9] Nanda, N. et al. (2023). "Progress measures for grokking via mechanistic interpretability." *ICLR 2023*.
