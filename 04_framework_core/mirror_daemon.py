...
class SurfaceCodeQEC:
    ...
    def __init__(self, code_distance: int = 3):
        ...
        self.n_x         = self.n_z
        log.debug(f"SurfaceCodeQEC initialized: d={code_distance}, "
...
class ExperimentLogger:
    ...
    def record(self, dp: DataPoint) -> None:
        ...
        self._writer.writerow([
            dp.step_id, dp.timestamp_ns, f"{dp.fidelity:.8f}",
            f"{dp.entropy:.8f}", f"{dp.logical_error_rate:.8f}",
            f"{dp.injection_magnitude:.8f}", int(dp.injection_approved),
            int(dp.correction_applied), f"{dp.noise_level:.6f}", dp.backend,
        ])
    ...
    def _setup_plot(self):
        ...
        self._axes[0].axhline(0.85, color="red", linestyle="--",
                               alpha=0.7, label="Feedback threshold")
    ...
    def _update_plot(self):
        ...
        ax0.axhline(0.85, color="red", linestyle="--", alpha=0.7, label="Feedback threshold")
        ax0.plot(steps, fids, "b-", linewidth=0.8, alpha=0.8, label="F(t)")
        if inj_steps:
            ax0.scatter(inj_steps, inj_fids, c="orange", s=20, zorder=5,
    ...
    def flush_h5(self) -> None:
        ...
        with h5py.File(self._h5_path, "w") as f:
            ...
            for field_name in ["step_id", "timestamp_ns", "fidelity", "entropy",
                                "logical_error_rate", "injection_magnitude",
                                "injection_approved", "correction_applied", "noise_level"]:
...
class MirrorDaemon:
    ...
    def initialize(self, reference_state: ComplexMatrix) -> None:
        ...
        log.info(f"Reference state entropy: "
    ...
    def step(self) -> DataPoint:
        ...
        with self._lock:
            ...
            if f < self.cfg.fidelity_threshold and self._step_count >= self.cfg.warmup_steps:
    ...
    def _print_summary(self, s: dict) -> None:
        ...
        print(f"  Injections       : {s.get('n_injections')} ({s.get('injection_rate', 0):.2%})")
        ...
        print(f"  S sign changes   : {s.get('entropy_sign_changes')}  ← non-monotonicity score")
        print(f"  Final λ          : {s.get('final_ler', 0):.8f}")
...
class StandardQECRunner:
    ...
    def run(self, n_steps: Optional[int] = None) -> dict:
        ...
        ds    = np.diff(s_arr)
        sign_changes = int(np.sum(np.diff(np.sign(ds)) != 0))
        log.info(f"Control run complete. F_final={fids[-1]:.6f} S_sign_changes={sign_changes}")
        return {
            "n_steps": len(data),
            "mean_fidelity": float(np.mean(fids)),
            "final_fidelity": float(fids[-1]),
...
if __name__ == "__main__":
    ...
    parser.add_argument("--noise",     type=float, default=0.004, help="Depolarizing noise p")
    parser.add_argument("--threshold", type=float, default=0.85,  help="Fidelity feedback threshold")
    ...
    parser.add_argument("--control",   action="store_true",        help="Run control (no feedback) only")
    ...
    if not args.control:
        ...
        backend_daemon = SimulatedBackend(
    ...
    if not args.control:
        ...
        if sc_d > sc_c and ff_d >= ff_c - 0.02:
            ...
            print("\n  ✗  No significant non-monotonicity detected.")
