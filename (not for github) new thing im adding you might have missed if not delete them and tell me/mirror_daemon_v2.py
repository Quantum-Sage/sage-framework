...
------------
  ...
  channel       │       │                                      │
                ...
                │       │                                      │
                │       ▼                                      │
                │  FeedForwardInjector (proportional α-blend)  │
                │       │                                      │
                │       ▼                                      │
                │  InjectionGuard (window-drift, Lyapunov)     │
                │       │                                      │
                │       ▼                                      │
...
1.  We do NOT use IIT Φ. It is not computable for quantum systems in any
    ...
    injection magnitude ‖δψ‖, Lyapunov exponent λ_L, and multiscale
    sample entropy SampEn(m, τ).
...
2.  The novel claim: normally QEC corrects toward a fixed target state.
    ...
    reference. Testable prediction: S should exhibit non-monotonic behavior
    (decrease during feedback, spike at re-injection) that is absent in
    standard QEC. Additionally, the entropy power spectrum should show a
    characteristic peak at the feedback cycle frequency.

3.  Critical failure mode: feedback injection magnitude growing unchecked.
    InjectionGuard handles this. Do not disable it.

4.  Hardware abstraction: swap backends without touching daemon logic.

5.  NEW — Adaptive threshold: under escalating noise, the threshold drops
    proportionally to d(noise)/dt, increasing feedback intervention rate.
    This is a control-theoretic extension that adds a second novel claim.
...
class SurfaceCodeSyndrome:
    ...
    def has_errors(self) -> bool:
        return bool(
            np.any(self.z_stabilizers == -1) or
            np.any(self.x_stabilizers == -1)
        )

    @property
    def error_weight(self) -> int:
        return int(
            np.sum(self.z_stabilizers == -1) +
            np.sum(self.x_stabilizers == -1)
        )
...
class SurfaceCodeQEC:
    ...
    def extract_syndrome(
        self, rho: ComplexMatrix, noise_level: float, round_id: int = 0,
    ) -> SurfaceCodeSyndrome:
        """Simulate syndrome extraction from density matrix."""
        off_diag  = float(np.abs(rho[0, 1]))
        flip_prob = noise_level * (1.0 - off_diag)
        z_synds = np.where(
            np.random.random(self.n_z) < flip_prob, -1, 1
        ).astype(np.int8)
        x_synds = np.where(
            np.random.random(self.n_x) < flip_prob, -1, 1
        ).astype(np.int8)
        return SurfaceCodeSyndrome(
            z_stabilizers=z_synds, x_stabilizers=x_synds,
            timestamp_ns=time.time_ns(), round_id=round_id,
        )
    ...
    def apply_correction(
        self, rho: ComplexMatrix, correction: ComplexMatrix,
    ) -> ComplexMatrix:
...
class SimulatedBackend(QuantumBackend):
    ...
    @property
    def name(self) -> str:
        return (f"SimulatedBackend(p={self.depolar_p}, "
    ...
    def step(self, injected_state: Optional[ComplexMatrix] = None) -> ChannelResult:
        ...
        return ChannelResult(
            state=rho, fidelity=f_val, noise_level=self.depolar_p,
            elapsed_ns=elapsed + self.step_duration_ns,
            step_id=self._step_count, metadata={"backend": "simulated"},
        )
...
class HostileBackend(QuantumBackend):
    ...
    def step(self, injected_state: Optional[ComplexMatrix] = None) -> ChannelResult:
        ...
        return ChannelResult(
            state=rho, fidelity=f_val, noise_level=self._current_noise,
            elapsed_ns=elapsed + self.step_duration_ns,
            step_id=self._step_count, metadata={"fatigue": self.fatigue},
        )
...
class InjectionGuard:
    ...
    def check(
        self, candidate: ComplexMatrix, reference: ComplexMatrix,
    ) -> tuple[bool, float]:
        """
    ...
    def summary(self) -> dict:
        return {
            ...
            "mean_norm":         float(np.mean(self._norm_history)) if self._norm_history else 0.0,
            "max_norm_seen":     float(np.max(self._norm_history)) if self._norm_history else 0.0,
...
class ExperimentLogger:
    ...
    CSV_FIELDS = [
        "step_id", "timestamp_ns", "fidelity", "entropy",
        "logical_error_rate", "injection_magnitude", "injection_approved",
        "correction_applied", "noise_level", "backend",
        "threshold_current", "bloch_x", "bloch_y", "bloch_z",
        "lyapunov_estimate",
    ]
    ...
    def record(self, dp: DataPoint) -> None:
        ...
        self._writer.writerow([
            dp.step_id, dp.timestamp_ns,
            f"{dp.fidelity:.8f}", f"{dp.entropy:.8f}",
            f"{dp.logical_error_rate:.8f}", f"{dp.injection_magnitude:.8f}",
            int(dp.injection_approved), int(dp.correction_applied),
            f"{dp.noise_level:.6f}", dp.backend,
            f"{dp.threshold_current:.6f}",
            f"{dp.bloch_x:.6f}", f"{dp.bloch_y:.6f}", f"{dp.bloch_z:.6f}",
    ...
    def _update_plot(self):
        ...
        if inj_steps:
            ax0.scatter(inj_steps, inj_fids, c="orange", s=10, zorder=5,
...
class MirrorDaemon:
    ...
    def __init__(
        ...
        self.threshold = AdaptiveThreshold(
            ...
            sensitivity=config.threshold_sensitivity if config.adaptive_threshold else 0.0,
        )
    ...
    def step(self) -> DataPoint:
        ...
        with self._lock:
            ...
            dp = DataPoint(
                ...
                bloch_x=bx, bloch_y=by, bloch_z=bz,
    ...
    def run(self, n_steps: Optional[int] = None) -> dict:
        ...
        try:
            for i in range(total):
                ...
                self.logger.record(DataPoint(
                    step_id=i, timestamp_ns=result.elapsed_ns,
                    fidelity=f, entropy=s, logical_error_rate=ler,
                    injection_magnitude=0.0, injection_approved=False,
                    correction_applied=corr_applied, noise_level=result.noise_level,
...
class StatisticalAnalyzer:
    ...
    @staticmethod
    def sample_entropy(
        series: np.ndarray, m: int = 2, r_frac: float = 0.2
    ) -> float:
        """
    ...
    def full_comparison(
        ...
        print(f"    Daemon:  Z={ww_d['Z']:.3f}  p={ww_d['p_value']:.4f}  → {ww_d['interpretation']}")
        print(f"    Control: Z={ww_c['Z']:.3f}  p={ww_c['p_value']:.4f}  → {ww_c['interpretation']}")
        ...
        print(f"    Scale | Daemon  | Control | Δ")
        ...
        if "status" in causal:
            ...
            print(f"    Mean ΔS after injection:     {causal['mean_ds_after_injection']:.6f}")
            print(f"    Mean ΔS after non-injection: {causal['mean_ds_after_non_injection']:.6f}")
            print(f"    t={causal['t_statistic']:.3f}  p={causal['p_value']:.4f}  d={causal['cohens_d']:.3f}")
...
class BlochTrajectoryPlotter:
    ...
    def plot_comparison(
        ...
        fig.suptitle("Bloch Sphere Trajectories: Daemon vs Control",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Mirror Daemon v2: adaptive QEC feedback")
    ...
    parser.add_argument("--fatigue",   type=float, default=0.08,  help="Noise escalation rate (HostileBackend)")
    parser.add_argument("--threshold", type=float, default=0.85,  help="Base fidelity threshold")
    parser.add_argument("--distance",  type=int,   default=3,     help="Surface code distance")
    parser.add_argument("--adaptive",  action="store_true",       help="Enable adaptive threshold")
    parser.add_argument("--backend",   choices=["simulated", "hostile"], default="hostile")
    ...
    parser.add_argument("--analyze",   action="store_true", default=True, help="Run statistical analysis")
    ...
    if result_daemon is not None:
        ...
        print(f"  Entropy sign changes — daemon: {sc_d:4d}  control: {sc_c:4d}  ratio: {sc_d/max(sc_c,1):.1f}x")
